    ...
    for (i, (emotion, prob)) in enumerate(zip(EMOTIONS, preds)):
               ...
               # emoji_face = feelings_faces[np.argmax(preds)]
                ...
                cv2.rectangle(frameClone, (fX, fY), (fX + fW, fY + fH),
                              (0, 0, 255), 2)
                emotion_cal[preds.argmax()]=emotion_cal[preds.argmax()]+1
#    for c in range(0, 3):
...
emotion_all=0
for i in range(7):
    print(str(EMOTIONS[i])+":"+str(emotion_cal[i])) #emotion 값 확인
    emotion_all+=emotion_cal[i]

happy_per = emotion_cal[3]/emotion_all
#print("전체 : "+ str(emotion_all))
#print("happy : "+str(emotion_cal[3]))
